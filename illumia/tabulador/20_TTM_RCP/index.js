...
const optionsData = {
  "CIERRE DE GESTION": {
    ...
    "Entrega con soporte": [],
    "Entrega sin soporte": [],
    "Entrega programada vigente": [],
    "Equipo entregado": [],
    "Posible entrega": [
      "PE - Cliente fuera de la cuidad",
      "PE - Lo tiene un tercero",
      "PE - Otros motivos",
      "NDE - Solicita visita para entrega",
      "NDE - Reclamo radicado DTV",
      "NDE - Fallecido",
      "NSR - Compro equipos",
      "NSR - Desconocimiento del servicio",
      "NSR - Fraude",
      "NSR - Perdida",
      "Otros Motivos"
    ],
...
resultadoSelect.addEventListener('change', function() {
    ...
    if (optionsData[tipoGestionSelect.value][selectedType])
    {
        ...
        let isEmptyArray = Array.isArray(options) && options.length == 0;
        
        if(!isEmptyArray)
        {
            options.forEach(item => feedBackSelect.innerHTML += createOptionElement(item));
